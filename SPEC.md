...
The **tool shape** toggle. `tool_shape`, `SLBH_TOOL_SHAPE` or `--tool-shape`
selects `lean` (the default: `apply_patch` and `bash`), `mid` (adds
`read_file`), `full` (slbh's original set, the default before 2026-09-21),
`anthropic` (Claude Code 2.1.278's `Bash`/`Read`/`Edit`/`Write`) or `codex`
(Codex 0.155.1's `exec_command`/`write_stdin`/`apply_patch`). A shape replaces
only the primary tools. `lean` became the default on the evidence of slb-org
`org/tool-lean-2026-09-21.md`: on GLM, `lean` and `mid` cost about 0.72×
`full`'s tokens at equal correctness. Every native shape describes
`apply_patch` accurately (unified diff via `git apply --recount`, or the Begin
Patch format), waits 10 s before backgrounding a command, and truncates
command output head-and-tail at 20,000 tokens. Captures of both foreign
harnesses, and the pins against them, live in
