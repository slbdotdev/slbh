...
func ToolDefinitions() []provider.Tool {
	...
	return []provider.Tool{
		...
		{Name: "launch_subagent", Description: "Launch a child agent up to depth two.", Parameters: map[string]any{"type": "object", "properties": map[string]any{"title": map[string]any{"type": "string"}, "harness": map[string]any{"type": "string"}, "model": map[string]any{"type": "string"}, "effort": map[string]any{"type": "string"}, "brief": map[string]any{"type": "string"}, "warn_after_seconds": map[string]any{"type": "integer"}, "working_dir": map[string]any{"type": "string"}, "ssh": map[string]any{"type": "string"}}, "required": []string{"title", "brief"}}},
...
func (r *Runtime) ExecuteTool(agentID, name, raw string) (string, error) {
	...
	case "launch_subagent":
		child, err := r.LaunchSubagentSpec(agentID, LaunchSpec{Title: value(a.Values, "title"), Harness: value(a.Values, "harness"), Model: value(a.Values, "model"), Effort: value(a.Values, "effort"), Brief: value(a.Values, "brief"), WarnAfterSeconds: intValue(a.Values, "warn_after_seconds"), WorkingDir: value(a.Values, "working_dir"), SSH: value(a.Values, "ssh")})
		if err != nil {
			return "", err
		}
		return child.ID, nil
