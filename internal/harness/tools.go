...
func ToolDefinitions() []provider.Tool {
	...
	return []provider.Tool{
		...
		{Name: "long_job", Description: "Start a non-blocking background shell job. Returns its job id immediately; the job's captured output is delivered to you automatically when it finishes, and cannot be read while it runs.", Parameters: map[string]any{"type": "object", "properties": map[string]any{"script": map[string]any{"type": "string"}, "cwd": map[string]any{"type": "string"}, "warn_after_seconds": map[string]any{"type": "integer", "description": "Seconds to wait before the harness sends YOU, the agent starting this job, one message saying the job is still running. That message is delivered into your context at the next API/tool call boundary and wakes you if you have gone idle; it does not go to a human. It is sent once and never repeated, and what to do about it is your decision: kill_job, keep waiting for the job's result, or carry on with other work. Defaults to 5 seconds."}}, "required": []string{"script"}}},
		...
		{Name: "long_py", Description: "Start a non-blocking background Python job in the managed scientific environment. Returns its job id immediately; the job's captured output is delivered to you automatically when it finishes, and cannot be read while it runs.", Parameters: map[string]any{"type": "object", "properties": map[string]any{"script": map[string]any{"type": "string"}, "cwd": map[string]any{"type": "string"}, "warn_after_seconds": map[string]any{"type": "integer", "description": "Seconds to wait before the harness sends YOU, the agent starting this job, one message saying the job is still running. That message is delivered into your context at the next API/tool call boundary and wakes you if you have gone idle; it does not go to a human. It is sent once and never repeated, and what to do about it is your decision: kill_job, keep waiting for the job's result, or carry on with other work. Defaults to 5 seconds."}}, "required": []string{"script"}}},
