...
func TestSystemPromptDirectsAsyncChildHandling(t *testing.T) {
	...
	for _, phrase := range []string{
		...
		"A subagent remains in the agent tree until end_subagent stops it.",
	...
	// The prompt states facts, not advice (owner, 2026-09-22): no thinking
	// paragraph and none of the behavioural rules it once carried.
	for _, dropped := range []string{"Keep your thinking brief", "never defer one", "Never sleep, poll", "End each subagent", "use it for temporary files"} {
		if strings.Contains(prompt, dropped) {
			t.Fatalf("seat system prompt still carries %q", dropped)
		}
	}
	...
	if !strings.Contains(leafPrompt, "next tool or API call boundary") {
		t.Fatalf("leaf prompt lost the shared mechanics: %s", leafPrompt)
	}
