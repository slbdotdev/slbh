...
func TestInternEffortDefaultEnvironmentAndPersistence(t *testing.T) {
	home := t.TempDir()
	t.Setenv("SLBH_HOME", home)
	t.Setenv("SLBH_INTERN_EFFORT", "")
	got := Load()
	if got.InternEffort != "low" {
		t.Fatalf("default InternEffort = %q", got.InternEffort)
	}

	t.Setenv("SLBH_INTERN_EFFORT", "high")
	got = Load()
	if got.InternEffort != "high" {
		t.Fatalf("environment InternEffort = %q", got.InternEffort)
	}
	...
	got = Load()
	if got.InternEffort != "medium" {
		t.Fatalf("persisted InternEffort = %q", got.InternEffort)
	}
}
